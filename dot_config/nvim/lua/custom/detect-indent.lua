...
-- Get value of option
local function opt(name) return vim.api.nvim_buf_get_option(0, name) end

-- Set value of option
local function setopt(name, value) return vim.api.nvim_buf_set_option(0, name, value) end

-- Get a line's contents as a string (0-indexed)
local function line_at(index) return vim.api.nvim_buf_get_lines(0, index, index + 1, true)[1] end
...
local function get_default_indent()
  if opt "expandtab" then
    ...
    if indent == 0 then indent = opt "tabstop" end
    return indent
  else
    return 0
  end
...
function M.setup(options)
  ...
  vim.api.nvim_create_autocmd({ "BufReadPost" }, {
    group = augroup,
    callback = function()
      vim.schedule(function() M.detect() end)
    end,
  ...
  vim.api.nvim_create_autocmd({ "BufNewFile" }, {
    ...
    callback = function()
      vim.api.nvim_create_autocmd("BufWritePost", {
        group = augroup,
        once = true,
        callback = function() M.detect() end,
      })
    end,
    desc = "Automatically detect indentation when saving new files",
  })
end
...
function M.detect()
  ...
  -- HACK: Excluded filetypes and buftypes
  local filetype_exclude = config("filetype_exclude", {
    "netrw",
    "tutor",
    "man",
    "qf",
  })
  local buftype_exclude = config("buftype_exclude", {
    "help",
    "nofile",
    "terminal",
    "prompt",
  ...
  -- HACK: Check if we should skip current buffer
  ...
  local is_multiline
  if skip_multiline then is_multiline = get_is_multiline_function() end
  ...
  while i ~= max_lines do
    ...
    if first_char == "\t" then
      ...
      if skip_multiline and is_multiline(i + 1) then goto continue end
  ...
  -- HACK: Show useful notifications
  if detected ~= default then
    if detected == 0 then
      setopt("expandtab", false)
      vim.notify("Set indentation for current buffer to tab.", vim.log.levels.INFO)
    else
      setopt("expandtab", true)
      setopt("tabstop", detected)
      setopt("softtabstop", detected)
      setopt("shiftwidth", detected)
      vim.notify(("Set indentation for current buffer to %s space(s)."):format(detected), vim.log.levels.INFO)
    end
  else
    if detected == 0 then
      vim.notify("Current buffer already uses tabs - no changes made.", vim.log.levels.INFO)
    else
      vim.notify(("Current buffer already uses %s space(s) - no changes made."):format(detected), vim.log.levels.INFO)
    end
  end
end
