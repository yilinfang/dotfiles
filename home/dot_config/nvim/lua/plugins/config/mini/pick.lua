...
if pick_ok then
  ...
  local function use_ripgrep_with_config(config_path, picker_function)
    ...
    -- Temporarily set the RIGPREP_CONFIG_PATH to the custom config path
  ...
  pick.registry.customized_files = function()
    use_ripgrep_with_config(vim.fn.stdpath "config" .. "/.rg/files", function() pick.builtin.files { tool = "rg" } end)
  end
  ...
  pick.registry.customized_files_all = function()
    use_ripgrep_with_config(
      ...
      function() pick.builtin.files { tool = "rg" } end
    )
  end
  vim.keymap.set(
    "n",
  ...
  pick.registry.customized_grep_live = function()
    use_ripgrep_with_config(
      ...
      function() pick.builtin.grep_live { tool = "rg" } end
    )
  end
