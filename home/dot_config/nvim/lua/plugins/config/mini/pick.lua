...
pick.registry.custom_rg_files = function(local_opts, opts)
  ...
  local_opts.tool = "rg"
  use_ripgrep_with_config(vim.fn.stdpath "config" .. "/.rg/files", function() pick.builtin.files(local_opts, opts) end)
end
...
pick.registry.custom_rg_grep = function(local_opts, opts)
  ...
  -- Force to use ripgrep with custom config
  local_opts.tool = "rg"
  use_ripgrep_with_config(vim.fn.stdpath "config" .. "/.rg/grep", function() pick.builtin.grep(local_opts, opts) end)
end
...
vim.keymap.set("n", "<leader>sg", "<cmd>Pick custom_rg_grep_live<cr>", { desc = "[S]earch by [G]rep" })
