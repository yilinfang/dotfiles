-- lua/custom/mark-signs.lua
-- Show marks in the statuscolumn
-- Borrowed from https://github.com/lewis6991/dotfiles/tree/main/config/nvim/lua/gizmos

local api = vim.api

api.nvim_set_hl(0, 'Custom.MarkSign', { link = 'ErrorMsg', default = true })
api.nvim_set_hl(0, 'Custom.MarkSignNum', { default = true })
api.nvim_set_hl(0, 'Custom.MarkSignPos', { default = true })
...
local function decor_mark(bufnr, mark)
  ...
  api.nvim_buf_set_extmark(bufnr, ns, row, 0, {
    ...
    sign_hl_group = 'Custom.MarkSign',
    number_hl_group = 'Custom.MarkSignNum',
  })
  -- Pcall in case the line length is zero
  pcall(api.nvim_buf_set_extmark, bufnr, ns, row, col, {
    end_col = col + off + 1,
    hl_group = 'Custom.MarkSignPos',
  })
end
